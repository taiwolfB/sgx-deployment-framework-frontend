...
function ResponsiveAppBar(props) {
  ...
  const handleClickDeployNewApplication = () => {
    props.setRenderNewDeployment(true);
  ...
  return (
    <AppBar position="static">
      <Container maxWidth="xl">
        <Typography sgx = {{mr: 2}}align='right'>{localStorage.getItem("loggedInUser")}</Typography>
        <Toolbar disableGutters>
          ...
          <Box sx={{ flexGrow: 1, display: { xs: 'none', md: 'flex' } }}>
            <Button
                key={pages[0]}
                onClick={handleClickDeployNewApplication}
              ...
              <Button
                key={pages[1]}
                onClick={handleClickViewDeployedApplications}
                sx={{ my: 2, color: 'white', display: 'block' }}
              >
                {pages[1]}
              </Button>
